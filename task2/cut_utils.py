...
class ResnetGenerator(nn.Module):
    ...
    """
    def __init__(self, im_dim=1, n_blocks=9, dropout=False):
        self.name = 'Generator'
...
class ProjectionHead(nn.Module):
    ...
    def forward(self, features, n_patches=64, idx_patch=None):
        ...
        """
        # indicies of selected patches for each feature
        i_select = [] 
        projections = []
        # initialize mlp
...
class PatchNCELoss(nn.Module):
    """PatchNCELoss
    modified from:
        https://github.com/taesungp/contrastive-unpaired-translation/models/patchnce.py
    """
    ...
    def forward(self, proj_A, proj_B, batch_size):
        ...
        # neg logit
        # for single-image translation, include the negatives from the entire minibatch.
        ...
        # diagonal entries are similarity between same features, and hence just fill the diagonal with very small number, which is exp(-10)
        diagonal = torch.eye(
...
# Utility functions    
def initialize_model_dict(epoch):
    model_dict = {
        'total_epoch': epoch,
